...
def detect_beaconing(df, min_connections=5, interval_std_threshold=5.0):
    df_copy = df.copy()
    grouped = df_copy.sort_values('timestamp').groupby(['src_ip', 'dst_ip'])
    df_copy['time_delta'] = grouped['timestamp'].diff().dt.total_seconds()
    stats = df_copy.sort_values('timestamp').groupby(['src_ip', 'dst_ip']).agg({"time_delta": "std", "action": "count"})
